...
def create_artificial_periods(campaigns):
    ...
    for i in range(len(all_dates) - 1):
        start = all_dates[i]
        end = all_dates[i+1] - pd.Timedelta(days=1)
        periods.append({
            'period_id': i + 1, 
            'start_date': start, 
            'end_date': end, 
...
def preprocess_data(full_run=True):
    ...
    train = train.merge(campaigns[['campaign_id', 'start_date', 'end_date']], on='campaign_id', how='left')
    train['start_date'] = pd.to_datetime(train['start_date'])
    train['period_id'] = pd.cut(train['start_date'], bins=period_bins).map(interval_to_id)
    train = train.dropna(subset=['period_id'])
    train['period_id'] = train['period_id'].astype(int)
    ...
    treatments = train.groupby(['customer_id', 'period_id', 'target_category']).size().unstack(fill_value=0)
...
def estimate_causal_effects(panel, treatment_col, feature_cols):
    """
    Executes Steps 3, 4, & 5: Causal Forest Estimation, ATE Validation, and GATE.
    """
    print(f"\n[{treatment_col}] Initiating Estimation Pipeline...")
    ...
    X = panel[X_cols].astype(float).fillna(0)
    Y = panel['avg_daily_expenditure'].astype(float).values
    T = panel[treatment_col].astype(int).values
    groups = panel['customer_id'].values
    
    # Propensity Score Trimming (Step 4)
    print(f"[{treatment_col}] Trimming bounds [0.01, 0.99] using strict Random Forest Propensities...")
    ...
    try:
        ...
        print(f"[{treatment_col}] Not enough treatment variation for cross-val propensities: {e}")
        return None
        
    mask = (p_hat >= 0.01) & (p_hat <= 0.99)
    print(f"[{treatment_col}] Retained {sum(mask)} out of {len(mask)} observations within common support.")
    ...
    if len(np.unique(T_trimmed)) < 2:
        print(f"[{treatment_col}] Insufficient treatment variance post-trimming. Skipping CATE.")
        return None
        
    # Step 3: Causal Forest Estimation (CATE)
    print(f"[{treatment_col}] Training CausalForestDML (n_estimators=2000, honest=True)...")
    est = CausalForestDML(
    ...
    print(f"[{treatment_col}] Calculating Doubly Robust ATE...")
    ate_summary = est.ate_inference(X_trimmed)
    print(f"[{treatment_col}] ATE Point Estimate: {ate_summary.mean_point:.3f} (p-value: {ate_summary.pvalue():.4f})")
    
    # Step 5: Group Average Treatment Effects (GATE)
    print(f"[{treatment_col}] Extracting CATEs for GATE Heterogeneity Validation...")
    cates = est.effect(X_trimmed).flatten()
    
    # Map out subgroup coefficient deviations using OLS
    # Using socio-demographic features and lagged spending quantiles
    demo_cols_X = [c for c in X_trimmed.columns if 'age_range' in c or 'income' in c or 'family_size' in c]
    
    if len(demo_cols_X) > 0:
        X_gate = X_trimmed[demo_cols_X]
        ...
        print(f"[{treatment_col}] GATE OLS R-squared: {gate_model.rsquared:.4f}")
...
def run_pipeline():
    ...
    # Target treatments for replication benchmarks (Step 7)
    # Based on prompt targets: Any Coupon, Drugstore, Other food, Other non-food.
    benchmarks = ['treatment_Any Coupon', 'treatment_drugstore items', 
                  'treatment_other food', 'treatment_other non-food products']
                  
    for t_col in benchmarks:
        if t_col in treatment_cols:
            # Exclude metadata from features X dynamically
            non_feature_cols = ['customer_id', 'start_date', 'end_date', 'duration_days', 
                                'total_expenditure', 'avg_daily_expenditure'] + treatment_cols
            feature_cols = [c for c in panel_full.columns if c not in non_feature_cols]
            
            res = estimate_causal_effects(panel_full, t_col, feature_cols)
            if res is not None:
                est, X_trimmed, Y_trimmed, T_trimmed, p_hat_trimmed, cates = res
                
                # Execute Optimal Policy Learning solely for the primary / Any Coupon test
                if t_col == 'treatment_Any Coupon':
    ...
    if t_col in treatment_cols_red:
        non_feature_cols_red = ['customer_id', 'start_date', 'end_date', 'duration_days', 
                                'total_expenditure', 'avg_daily_expenditure'] + treatment_cols_red
        feature_cols_red = [c for c in panel_reduced.columns if c not in non_feature_cols_red]
        estimate_causal_effects(panel_reduced, t_col, feature_cols_red)
